...
/**
 * Resolve a demo fixture's current public slug by numeric id. Slugs are opaque
 * random tokens regenerated by the migration — never hardcode them in tests.
 */
async function slugOfCourse(id) {
  const c = await prisma.course.findUnique({ where: { id }, select: { slug: true } });
  if (!c) throw new Error(`fixture course ${id} missing`);
  return c.slug;
}
async function slugOfVideo(id) {
  const v = await prisma.bunnyVideo.findUnique({ where: { id }, select: { slug: true } });
  if (!v) throw new Error(`fixture video ${id} missing`);
  return v.slug;
}

async function makeStudent(tag) {
  const email = `seed-${tag}-${Date.now()}@localhost.test`;
  const stu = await prisma.user.create({ data: { slug: randomBase36Slug(), name: 'Seed', email, password: 'x', grade: 'FIRST_SECONDARY' } });
  const cookie = `accessToken=${createToken({ id: stu.id, email, name: 'S', role: 'STUDENT' }, config.jwt.secret)}`;
  const admin = adminCookie();
  const enr = await req('POST', '/admin/enrollments', admin, { userSlug: stu.slug, courseSlug: await slugOfCourse(1) });
  assert.equal(enr.status, 201, 'enroll fixture');
  const grant = await req('POST', `/quizzes/videos/${await slugOfVideo(1)}/exemptions`, admin, { userSlug: stu.slug, reason: 'seed' });
  assert.equal(grant.status, 200, 'exemption fixture');
  await req('POST', '/progress/complete', cookie, { videoSlug: await slugOfVideo(2) });
  return {
    stu, cookie,
...
describe('quiz lifecycle', () => {
  ...
  it('hide-until-pass: failed attempt reveals scores only, retake allowed', async () => {
    ...
    try {
      const start = await req('POST', `/quizzes/videos/${await slugOfVideo(2)}/start`, fx.cookie, {});
      ...
      const retry = await req('POST', `/quizzes/videos/${await slugOfVideo(2)}/start`, fx.cookie, {});
  ...
  it('Q-5: mid-flight key edit cannot re-grade an in-flight attempt', async () => {
    ...
    try {
      ...
      const flip = await req('POST', `/quizzes/videos/${await slugOfVideo(2)}`, adminCookie(),
      ...
      try {
        ...
        const restore = await req('POST', `/quizzes/videos/${await slugOfVideo(2)}`, adminCookie(),
  ...
  it('gates: passed quiz blocks retake, locked video denies playback', async () => {
    ...
    const start = await req('POST', `/quizzes/videos/${await slugOfVideo(1)}/start`, seqCookie, {});
    ...
    const gate = await req('GET', `/videos/${await slugOfVideo(2)}/playback`, graderCookie);
