...
const getVideosByCourse = async (req, res) => {
  try {
    ...
    const videosWithUrls = videos.map((video) => {
      ...
      if (video.muxData && video.muxData.playbackId) {
        result.streamUrl = `https://stream.mux.com/${video.muxData.playbackId}.m3u8`;
      } else {
...
const getVideoById = async (req, res) => {
  try {
    ...
    if (video.muxData && video.muxData.playbackId) {
      video.streamUrl = `https://stream.mux.com/${video.muxData.playbackId}.m3u8`;
    } else {
...
// Upload a new video with Mux
const uploadVideo = async (req, res) => {
  try {
    const { courseId } = req.params;
    const { title, duration, muxAssetId } = req.body;

    if (!title || !muxAssetId) {
      return res.status(400).json({ error: 'Title and Mux Asset ID are required' });
    }
    ...
    const video = await prisma.video.create({
      data: {
        ...
        thumbnail: '', // Thumbnail can be generated by Mux
        muxData: {
          create: {
            assetId: muxAssetId,
            playbackId: '', // This will be populated by the webhook
          },
        },
      },
      include: {
    ...
    res.status(201).json({ message: 'Video created successfully. Waiting for Mux processing.', video });
  } catch (error) {
    console.error('Error creating video with Mux:', error);
