...
const getVideosByCourse = async (req, res) => {
  try {
    ...
    const videosWithUrls = videos.map((video) => {
      ...
      // Set URL for all videos
      result.url = video.url && !video.url.startsWith('http') ? `${baseUrl}/${video.url}` : video.url;
...
const uploadVideo = async (req, res) => {
  try {
    ...
    if (!course) {
      ...
      if (req.files.thumbnail && fs.existsSync(thumbnailPath)) {
...
// Legacy upload video function
const legacyUploadVideo = async (req, res) => {
  try {
    const { courseId } = req.params;
